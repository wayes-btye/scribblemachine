...
try {
            ...
            if ($processId) {
                try {
                    ...
                    if ($process) {
                        ...
                        Write-Host "Found process $processName (PID: $processId) using port 3000" -ForegroundColor Red
    ...
    $output = cmd /c "netstat -ano | findstr :3000 | findstr LISTENING"
    if ($output) {
        ...
        if ($processId -match '^\d+$') {
            Write-Host "Found process $processId using port 3000" -ForegroundColor Red
            Stop-Process -Id $processId -Force -ErrorAction SilentlyContinue
            Write-Host "Killed process $processId" -ForegroundColor Green
        } else {
